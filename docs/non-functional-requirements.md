...
| **Observability — logging** | **Python stdlib `logging`**, per-component loggers (`zylabs.workflow`, `zylabs.runner`, `zylabs.api`, `zylabs.chat`), full tracebacks on failure paths | Ship logs to an aggregator (Loki/Datadog) + request IDs | Structured logging on failure paths is already in place. |
